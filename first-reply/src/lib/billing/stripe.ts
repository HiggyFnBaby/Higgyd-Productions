...
export const stripeProvider: BillingProvider = {
  ...
  async createCheckoutSession({ agentId, customerEmail, successUrl, cancelUrl }: CheckoutSessionParams) {
    ...
    const session = await client().checkout.sessions.create({
      ...
      // metadata is how the webhook later maps a Stripe event back to an
      // agent — Stripe has no concept of "agent" on its own.
      metadata: { agentId },
      subscription_data: { metadata: { agentId } },
    });
